...
- **LAN**: 有線LANの送受信速度（Mbps）。接続されていない場合は`--`。
- **WIFI**: Wi-Fiの送受信速度（Mbps）。接続されていない場合は`--`。
...
| 設定を初期化 | 表示ラベル、最前面表示、自動実行、履歴画面の位置とサイズを初期状態に戻します。 |
| ログフォルダーを開く | 診断ログを保存しているフォルダーを開きます。 |
| 最新版を確認 | GitHubの最新版リリースページを開きます。 |
| 終了 | アプリを終了します。 |

負荷が70%以上になるとアイコンが黄色、90%以上になると赤色に変わります。

表示ラベル、「最前面に固定」、履歴画面の位置・サイズは、`%LOCALAPPDATA%\\TaskbarStats\\settings.json` に保存され、次回起動時に復元されます。履歴グラフのデータ自体は終了時に保存されません。初回起動時には、起動したことを知らせる通知が1回だけ表示されます。
...
1. タスクマネージャーで`TaskbarStats.exe`が既に起動していないか確認します。既に起動している場合は、二重起動を知らせるメッセージが表示されます。
